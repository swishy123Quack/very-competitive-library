...
\algorithm{Stress Tester (Linux)}
{Simple .sh file for stress testing.}
{sh}{swishy/StressTester.sh}
...
\algorithm{Eulerian Path/Cycle}{
    Find a path/cycle that traverse every edges exactly once.
}
[$\mathcal{O}(N+M)$]  
{cpp}{swishy/EulerianPath.cpp}
...
\algorithm{Floor Sum}{
    $sum^{n-1}_0$ floor($(a * i + b) / m$)
    (only works for $a, b >= 0$!!!)
}
[$\mathcal{O}(\log(n + m + a + b))$]
{cpp}{lenhanbo/floor_sum.cpp}
...
\algorithm{XOR Convolution}{
    \begin{itemize}
        \item We want to transform a[x] $\rightarrow$ b[x] to perform pure multiple operation.
...
\subsection{Cayley Theorem}
    Number of labeled vertice $n$ nodes tree is $n^(n-2)$
...
\item Apollonius Circle

The set $z : \left| z - a \right| = k \left| z - b \right|$ is the circle of radius $\frac{k\left|b-a\right|}{\lvert k^2-1 \rvert}$ centered at $\frac{k^2b-a}{k^2-1}$.
