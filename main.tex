...
\algorithm{CCW}{
    \begin{itemize}
        ...
        \item P is supposed to be Point<T> where T is e.g. double or long long.
        \item It uses products in intermediate steps so watch out for overflow if using int or long long.
    \end{itemize}
}
{cpp}{kactl/CCW.cpp}
...
\algorithm{Circle Line}{
    Finds the intersection between a circle and a line. Returns a vector of either 0, 1, or 2 intersection points. P is intended to be Point<double>.
}
[$\mathcal{O}(1)$]
...
\algorithm{Circle Tagents}{
    Finds the external tangents of two circles, or internal if r2 is negated. Can return 0, 1, or 2 tangents -- 0 if one circle contains the other (or overlaps it, in the internal case, or if the circles are the same);
