...
pair<ll, ll> CRT(ll x1, ll m1, ll x2, ll m2) { // repeatively do this on all modulo to get final result
  ll g = gcd(m1, m2);
  ...
  return make_pair((res % lcm + lcm) % lcm, lcm); // return x mod (lcm(m1,m2,...mi))
}
